...
from .build_items import build_mat_node, build_prod_node, build_invent_prod_mat, build_invent_mat, build_copy_prod

def parse_build_chain(product_item, conn):
    products = [build_prod_node(product_item, product_item.activity.activity_type)]
    materials = [build_mat_node(material, product_item.activity.activity_type) for material in product_item.activity.material]
    ...
    for invent_product_item in invent_product_items:
        ...
        materials.append(build_invent_prod_mat(invent_product_item))
    ...
    products = concat(products, axis=0).drop_duplicates().reset_index(drop=True)
    materials = concat(materials, axis=0).drop_duplicates().reset_index(drop=True)
